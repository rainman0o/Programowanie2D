...
var endedTaksWithId = tasks.Where(t => t.IsCompleted == true).Select((t, index) =>
new {t = tasks.Select(p => p.Name), index = tasks.Select(p => p.Id)});
foreach(var r in endedTaksWithId)
{
    Console.WriteLine($"{r.t}, {r.index}");
}
